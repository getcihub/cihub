...
// content key pattern used in the cache, comprised of
// installation name and user login.
...
func (c *cacher) Membership(ctx context.Context, user *core.User, name string) (bool, bool, error) {
	...
	if ok {
		item := cached.(*item)
		// check if the item is expired and can be ejected
		// from cache, else return the result.
	...
	// get up-to-date membership details due to cache miss
	// or expired cache item.
