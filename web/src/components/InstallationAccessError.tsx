...
export function InstallationAccessError({ status, reason }: InstallationAccessErrorProps) {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen">
            <Card className={`${bgColor} ${borderColor} p-8 text-center max-w-2xl w-full mx-4`}>
                <Icon className={`size-12 ${iconColor} mx-auto mb-4`} aria-hidden="true" />
                <h1 className={`text-2xl font-bold ${textColor} mb-2`}>{title}</h1>
                <p className={textColor}>{description}</p>
                {reason && (
                    <p className={`text-sm ${textColor} mt-3 opacity-75`}>
                        Error reason: <span className="font-mono">{reason}</span>
                    </p>
                )}
