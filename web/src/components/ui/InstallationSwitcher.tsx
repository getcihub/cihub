...
import { RiCheckLine, RiArrowDownSLine, RiAddLine } from '@remixicon/react'
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuGroup,
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                ...
                >
                    <div className="flex items-center gap-2 min-w-0">
                        {selectedInstallation.avatar_url ? (
                            ...
                            <div className="size-6 rounded-md bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center flex-shrink-0 text-white font-semibold text-xs">
                                {selectedInstallation.login.charAt(0).toUpperCase()}
                            </div>
                        )}
                    ...
                    <RiArrowDownSLine className="size-4 text-gray-500 flex-shrink-0" aria-hidden="true" />
                </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-56">
                ...
                <DropdownMenuGroup>
                    {installations.length > 0 ? (
                        installations.map((installation) => (
                            <DropdownMenuItem
                                key={installation.id}
                                onSelect={() => handleSelectInstallation(installation.login)}
                            ...
                            >
                                ...
                                <span className="flex-1 truncate">{installation.login}</span>
                                {selectedInstallation.id === installation.id && (
                                    <RiCheckLine className="size-4 text-green-600 flex-shrink-0" aria-hidden="true" />
                                )}
                            </DropdownMenuItem>
                        ))
                    ) : (
                ...
                <DropdownMenuGroup>
                    ...
                    >
                        <RiAddLine className="size-4 flex-shrink-0" aria-hidden="true" />
