...
export function JobDetailPage() {
    ...
    const handleMachineClick = () => {
        if (selectedInstallation && assignedRunner) {
            navigate({ to: '/$login/machines/$name', params: { login: selectedInstallation.login, name: assignedRunner.machine } })
        }
    }
    ...
    const queueWaitTime = job && job.created > 0 && job.started > 0
        ? job.started - job.created
        : 0

    // Calculate total time
    const totalTime = job && job.created > 0 && job.completed > 0
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                        ...
                        {job.conclusion && (
                            <span className="text-sm text-gray-600">
                                Conclusion: <span className="font-medium capitalize">{job.conclusion}</span>
                            </span>
                        )}
                    </div>
                </div>
                    ...
                    <Card className="p-6">
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <div className="mt-2">
                                    <p className="text-2xl font-bold text-gray-900">{formatDuration(job.created, job.started)}</p>
                                    <p className="text-xs text-gray-500 mt-1">Time before execution</p>
                                </div>
                            </div>
                            <div className="rounded-lg bg-yellow-100 p-3">
                                <RiTimeLine className="size-6 text-yellow-600" aria-hidden="true" />
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="p-6">
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <div className="mt-2">
                                    <p className="text-2xl font-bold text-gray-900">{formatDuration(job.started, job.completed)}</p>
                                    <p className="text-xs text-gray-500 mt-1">Actual job duration</p>
                                </div>
                            </div>
                            <div className="rounded-lg bg-purple-100 p-3">
                                <RiCheckLine className="size-6 text-purple-600" aria-hidden="true" />
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="p-6">
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <div className="mt-2">
                                    <p className="text-2xl font-bold text-gray-900">{formatDuration(job.created, job.completed)}</p>
                                    <p className="text-xs text-gray-500 mt-1">{queueWaitTime > 0 ? Math.round((queueWaitTime / totalTime) * 100) : 0}% queue wait</p>
                                </div>
                            </div>
                            <div className="rounded-lg bg-blue-100 p-3">
                                <RiTimeLine className="size-6 text-blue-600" aria-hidden="true" />
                            </div>
                        </div>
                    </Card>
                </div>
            ...
            {/* Timeline Card */}
            <Card className="p-6">
                <h2 className="text-sm font-semibold text-gray-900 mb-4">Job Timeline</h2>
                <div className="space-y-3">
                    ...
                    <div className="flex gap-3">
                        <div className="flex flex-col items-center flex-shrink-0">
                            ...
                            {(job.queued > 0 || job.started > 0 || job.completed > 0) && (
                        ...
                        <div className="flex-1 pt-0.5 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                                {new Date(job.created * 1000).toLocaleString('en-US', {
                                    month: 'short',
                                    day: 'numeric',
                                    hour: '2-digit',
                                    minute: '2-digit',
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="flex-1 pt-0.5 min-w-0">
                                <div className="flex items-center justify-between gap-2">
                                    <p className="text-xs font-semibold text-gray-900">Queued</p>
                                    <span className="text-xs text-gray-500 flex-shrink-0">
                                        +{formatDuration(job.created, job.queued)}
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 mt-0.5">
                                    {new Date(job.queued * 1000).toLocaleString('en-US', {
                                        month: 'short',
                                        day: 'numeric',
                                        hour: '2-digit',
                                        minute: '2-digit',
                    ...
                    {job.completed > 0 && (
                        <div className="flex gap-3">
                            ...
                            <div className="flex-1 pt-0.5 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 mt-0.5">
                                    {new Date(job.completed * 1000).toLocaleString('en-US', {
                                        month: 'short',
                                        day: 'numeric',
                                        hour: '2-digit',
                                        minute: '2-digit',
                ...
                <Card className="p-6">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-semibold text-gray-900 mb-4">Runner Assignment</h3>
                            <div className="space-y-3">
                                <div className="pb-3 border-b border-gray-200">
                                    <p className="text-xs font-medium text-gray-600">Runner</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <p className="text-sm font-medium text-gray-900">{assignedRunner.name}</p>
                                ...
                                <div className="pb-3 border-b border-gray-200">
                                    <p className="text-xs font-medium text-gray-600">Machine</p>
                                    <button
                                ...
                                <div>
                                    <p className="text-xs font-medium text-gray-600 mb-3">Architecture & Resources</p>
                                    <div className="flex flex-wrap gap-4">
                                        <div className="flex items-center gap-2">
                                            <span className="text-gray-400">
                                                {assignedRunner.arch === 'x86_64' ? 'x86' : assignedRunner.arch === 'arm64' ? 'ARM' : assignedRunner.arch}
                                            </span>
                ...
                <Card className="p-6">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            <h3 className="text-sm font-semibold text-gray-900">Runner</h3>
                            <p className="text-sm text-gray-900 mt-2">{job.runner_name}</p>
                        </div>
                    </div>
                </Card>
            ) : null}
